...
def generate_ortools_code(model_def: dict) -> str:
    ...
    obj_expr = _build_expr(obj_coeffs, var_map)
    if obj_expr == "0":
        L.append("    # 目标函数系数均为零")
        obj_expr = "0"
    ...
    for idx, c in enumerate(constraints):
        ...
        c_var = f"c{idx + 1}"
        lhs_expr = _build_expr(c_coeffs, var_map)
    ...
    for v in variables:
        name = v.get("name", "x")
        safe = var_map.get(name, _safe_var(name))
...
def _build_expr(coefficients, var_map: Dict[str, str]) -> str:
    """
    ...
    if isinstance(coefficients, list):
        ...
        for item in coefficients:
            ...
            if abs(c) > 1e-10:
                safe = var_map.get(vn, _safe_var(vn))
                terms.append((safe, c))
        ...
        if i == 0:
            ...
            if coeff > 0:
                ...
                if abs_c == 1:
                    ...
                    parts.append(f"- {_fmt_num(abs_c)} * {name}")
