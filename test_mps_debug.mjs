...
function simulateHandleApply(parsedResult) {
  const mappings = {};
  parsedResult.variables.forEach(v => mappings[v.name] = '');
  
  const mappedVariables = parsedResult.variables.map((varData, idx) => {
    const mapping = mappings[varData.name];
    if (mapping) {
      const [ontologyId, objectId, fieldId] = mapping.split('::');
      return {
        id: `v-file-${idx + 1}`,
        name: varData.name,
        source: 'ontology',
        ontologyRef: { ontologyId, objectId, fieldId },
  ...
  const varIdMap = {};
  mappedVariables.forEach(v => { varIdMap[v.name] = v.id; });

  const mappedConstraints = parsedResult.constraints.map((c, idx) => {
    const coefficients = {};
    for (const [varName, coeff] of Object.entries(c.coefficients)) {
      const varId = varIdMap[varName];
      if (varId) {
        coefficients[varId] = coeff;
      }
    }
    return {
      id: `c-file-${idx + 1}`,
  ...
  for (const [varName, coeff] of Object.entries(parsedResult.objective.coefficients)) {
    const varId = varIdMap[varName];
    if (varId) {
      objCoefficients[varId] = coeff;
    }
  }

  return {
    problemType: parsedResult.variables.some(v => v.isInteger) ? 'MIP' : 'LP',
    variables: mappedVariables,
    objective: {
      sense: parsedResult.objective.sense,
      coefficients: objCoefficients,
    },
    constraints: mappedConstraints,
  };
}

const text = readFileSync('/Users/shijinxin/PycharmProjects/or_solver/python_HiGHs/aps_example/aps_example.mps', 'utf8');
const parsedResult = parseMPSFile(text);

console.log('=== 解析结果 ===');
console.log('目标函数系数:', JSON.stringify(parsedResult.objective.coefficients));
console.log('变量数量:', parsedResult.variables.length);
console.log('约束数量:', parsedResult.constraints.length);

console.log('\n=== 模拟应用到模型 ===');
const modelData = simulateHandleApply(parsedResult);
console.log('目标函数系数:', JSON.stringify(modelData.objective.coefficients));
console.log('目标方向:', modelData.objective.sense);
console.log('变量数量:', modelData.variables.length);
console.log('约束数量:', modelData.constraints.length);

console.log('\n=== 变量ID映射 ===');
modelData.variables.forEach(v => console.log(`${v.name} -> ${v.id}`));

console.log('\n=== 目标函数系数（使用变量ID）===');
console.log(JSON.stringify(modelData.objective.coefficients));
